    ...
    def post(self, request, pk):
        ...
        if page is None:
            page = Page.objects.create(manga=m, number=m.pages.count() + 1,
                                       author=request.user,
                                       source=SOURCE_HUMAN)
        # Store the generated bytes as a FILE. Assigning the data URI to
        # art_url worked on sqlite and raised DataError on Postgres, so
        # generated art would have failed in production only.
        drawn = _decode_data_uri(image)
        if drawn is not None:
            page.art = drawn
        else:
            page.art_url = image[:500]
        page.art_source = SOURCE_CHARACTER
        page.ai_prompt = {**(page.ai_prompt or {}), "art": record}
        page.ai_model = model_name
        page.save(update_fields=["art", "art_url", "art_source", "ai_prompt",
                                 "ai_model"])
...
def _decode_data_uri(value):
    """Turn a canvas data URI into a Django file, or return None.

    This is what a finger or a mouse produces: the browser hands you
    `data:image/png;base64,...` off a <canvas>. Decoded and stored as a real
    file rather than kept as text — a drawn page is hundreds of kilobytes of
    base64, and a text column is not where that belongs.
    """
    import base64
    import uuid
...
class PageArtView(APIView):
    """Draw a page yourself, or upload one you already made.

    Three ways art gets onto a page and this covers the two that are yours:

        drawn    a finger or a mouse — the client posts the canvas as a data URI
        upload   a file you already had, as multipart

    The AI route is elsewhere and costs money; these two are free and always
    will be. Drawing your own page is the whole point of a manga app, and
    charging for it would be charging somebody to use their own hands.

    Neither counts as AI, so a page you drew owes no royalty regardless of
    what wrote the script.
    """
    ...
    def post(self, request, pk, number):
        ...
        upload = request.FILES.get("art")
        drawn = _decode_data_uri((request.data or {}).get("image"))
        if not upload and not drawn:
            return Response(
                {"detail": "Send an image — either a file as `art`, or your "
                           "canvas as `image` (a data: URI).",
                 "accepts": ["multipart file field 'art'",
                             "JSON 'image' as data:image/png;base64,…"]},
                status=status.HTTP_400_BAD_REQUEST)

        page, _created = Page.objects.get_or_create(
            manga=m, number=max(1, int(number)),
            defaults={"author": request.user, "source": SOURCE_HUMAN})
        page.art = upload or drawn
        page.art_source = ART_UPLOAD if upload else ART_DRAWN
        # Replacing AI art with your own drawing has to clear the AI record,
        # or the work keeps paying a royalty on a page a person drew.
        if page.ai_prompt:
            page.ai_prompt = {k: v for k, v in page.ai_prompt.items()
                              if k != "art"}
        page.save(update_fields=["art", "art_source", "ai_prompt"])
        return Response({"page": {"number": page.number,
                                  "art": page.art_src(),
                                  "art_source": page.art_source,
                                  "ai_assisted": page.ai_assisted},
    ...
    def delete(self, request, pk, number):
        """Rub it out and start again."""
        page = Page.objects.filter(manga_id=pk, number=number,
                                   manga__room__members__user=request.user).first()
        if not page:
            return Response({"detail": "No such page."},
                            status=status.HTTP_404_NOT_FOUND)
        page.art.delete(save=False)
        page.art = None
